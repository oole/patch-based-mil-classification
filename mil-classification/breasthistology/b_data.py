...
def getlabel(patch):
    ...
    if "Benign" in patch:
        label = "Benign"
    elif "InSitu" in patch:
        label = "InSitu"
    elif "Invasive" in patch:
        label = "Invasive"
    elif "Normal" in patch:
        label = "Normal"
    return label
...
def labelencoder():
    lE = LabelEncoder()
    lE.fit(["Benign", "InSitu", "Invasive", "Normal"])
    return lE
...
def collect_data(datapath):
    ...
    slideData = SlideData(slidelist, None, patchnumber, slide_label, getlabel, True,
                          ...
                          parseFunctionAugment=b_image_augment,
                          parseFunction=b_image_noaugment)
    return slideData

def b_image_augment(img_path, img_label):
    one_hot = tf.one_hot(img_label, 4)
    ...
    img_crop = tf.image.crop_to_bounding_box(img_saturation, 0,0,128, 128)
    img_standard = tf.image.per_image_standardization(img_crop)
    return img_standard, one_hot

def b_image_noaugment(img_path, img_label):
    one_hot = tf.one_hot(img_label, 4)
    ...
    img_float = tf.image.convert_image_dtype(img_decoded, tf.float32)
    # End add more augmentation
    img_crop = tf.image.crop_to_bounding_box(img_float, 0,0,128, 128)
    img_standard = tf.image.per_image_standardization(img_crop)
    return img_standard, one_hot