...
def train_given_net(iterator_handle, train_iterator_handle, val_iterator_handle,
                    train_iterator_len, val_iterator_len, train_iterator, val_iterator,
                  train_op, loss_op, accuracy_op, keep_prob, learning_rate, is_training,
                  num_epochs=2, batch_size=64, dropout_ratio=0.5, lr=0.0005, sess=tf.Session):
    ...
    for epoch in range(num_epochs):
        ...
        while True:
            try:
                ...
                util.update_print(
                    "Training, Epoch: %0.f -- Loss: %0.5f, Acc: %0.5f, %0.d / %0.d" %
                    (epoch+1, err, acc, i, len(train_iterator_len) // batch_size + 1))
                i = i + 1
                batch_train_acc.append(acc)
                batch_train_err.append(err)
            except tf.errors.OutOfRangeError:
