...
import lymphoma.l_data as dataUtil
import lymphoma.l_net as netUtil
import os
import train
import train_em
...
BASENAME = "lymph_eval"
SIMPLERUNSTAMP = "190518_l-" + str(LEARNING_RATE) + "_drop-" + str(DROPOUT_RATIO) + "_bs-" + str(
    BATCH_SIZE) + "_allInOne-second"
...
def evaluate(numberOfEpochs=30):
    ...
    trainSlideData = dataUtil.collect_data(trainDataPath)
    testSlideData = dataUtil.collect_data(testDataPath)
    ...
    _, _, netAcc = train.train_net(trainSlideData, testSlideData,
                                   ...
                                   getlabel_train=dataUtil.getlabel,
                                   runName=runName,
                                   lr=LEARNING_RATE,
                                   buildNet=netUtil.getLymphNet,
                                   valIsTestData=True,
